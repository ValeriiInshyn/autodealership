...
Go
create or alter procedure dbo.staging_to_OLAP_inremental
as
begin
-- Job variables
...
merge CompStoreStorage.dbo.device_color_dim as trg
using 
	(select name, color_id as bk_color_id 
	from CompStoreStaging.dbo.color) as src
on
	(src.bk_color_id = trg.bk_color_id)
when matched then
	update set 
	name = src.name
when not matched then
	insert(name, bk_color_id)
	values(src.name, src.bk_color_id);
...
merge CompStoreStorage.dbo.store_dim as trg
using 
	...
	join CompStoreStaging.dbo.country cntr on ct.country_id = cntr.country_id) as src
on
	(src.br_store_id = trg.br_store_id)
when matched then
	update set
	name = src.name,
	city = src.city,
	...
	address = src.address
when not matched then
	insert(name, city, country, address, br_store_id)
	values(src.name, src.city, src.country, src.address, src.br_store_id);
...
merge CompStoreStorage.dbo.computer_device_dim as trg
using 
	(select 
	...
	join CompStoreStaging.dbo.computer_device_subtype cds on cd.computer_device_subtype_id = cds.computer_device_subtype_id) as src
on 
    (src.bk_computer_device_id = trg.bk_computer_device_id)
when matched then
	update set 
	...
	computer_device_subtype = src.computer_device_subtype,
	model = src.model
when not matched then 
	insert(color_id, material, price, manufacturer, computer_device_subtype, model, bk_computer_device_id)
	values(src.color_id, src.material, src.price, src.manufacturer, src.computer_device_subtype, src.model, src.bk_computer_device_id);
...
merge CompStoreStorage.dbo.supply_dim as trg
using 
	(select
	...
	join CompStoreStorage.dbo.date_dim dd on
		DAY(s.supply_date)=dd.day and MONTH(s.supply_date)=dd.month and YEAR(s.supply_date)=dd.year) as src
on 
    (src.bk_supply_id = trg.bk_supply_id)
when matched then
	update set
	supply_number = src.supply_number,
	amount = src.amount,
	supply_date_id = src.supply_date_id,
	status = src.status
when not matched then 
	insert(supply_number, amount, supply_date_id, status, bk_supply_id)
	values(src.supply_number, src.amount, src.supply_date_id, src.status, src.bk_supply_id);
...
using 
	...
	join CompStoreStorage.dbo.date_dim dd on
		DAY(sh.shipment_date)=dd.day and MONTH(sh.shipment_date)=dd.month and YEAR(sh.shipment_date)=dd.year) as src
on 
    (src.supply_id = trg.supply_id and src.store_id = trg.store_id and src.computer_device_id = trg.computer_device_id)
when matched then
	update set
	supply_id = src.supply_id,
	store_id = src.store_id,
	computer_device_id = src.computer_device_id,
	shipment_date_id = src.shipment_date_id,
	previous_shipment_date_id = src.previous_shipment_date_id,
	shipment_period = src.shipment_period,
	price = src.price,
	quantity = src.quantity,
	fraction_of_supply_cost = src.fraction_of_supply_cost
when not matched then 
	insert(supply_id, store_id, computer_device_id, shipment_date_id, previous_shipment_date_id, shipment_period, price, quantity, fraction_of_supply_cost)
	values(src.supply_id,src.store_id,src.computer_device_id,src.shipment_date_id,src.previous_shipment_date_id,src.shipment_period,src.price,src.quantity,src.fraction_of_supply_cost);
...
merge CompStoreStorage.dbo.order_details_fact as trg
using 
	(select
	c_dim.color_id as color_id,
	...
	group by s_dim.store_id, c_dim.color_id, sd.date_id, ed.date_id) as src
on 
    (src.store_id = trg.store_id and src.color_id = trg.color_id 
	and src.start_period_date_id = trg.start_period_date_id
	and src.end_period_date_id = trg.end_period_date_id)
when matched then
	update set
	color_id = src.color_id,
	store_id = src.store_id,
	start_period_date_id = src.start_period_date_id,
	end_period_date_id = src.end_period_date_id,
	order_count = src.order_count,
	unique_client_count = src.unique_client_count
when not matched then 
	insert(color_id, store_id, start_period_date_id, end_period_date_id, order_count, unique_client_count)
	values(src.color_id,src.store_id,src.start_period_date_id,src.end_period_date_id,src.order_count,src.unique_client_count);
